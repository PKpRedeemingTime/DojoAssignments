def filterByType(arr):
    ...
    while count < len(arr): #creates a while statement to run through the list
        if isinstance(arr[count], int) == True: #checks if each value in the list is an integer
            number += arr[count] #if the value is an integer, the value of the integer is added to our integer variable 'number'
        elif isinstance(arr[count], float) == True:
            number += arr[count] #if the value is a number with a decimal, the value of the integer is added to our integer variable 'number'
        elif isinstance(arr[count], str) == True: #checks if each value in the list is a string
            sentence = sentence + " " + arr[count] #if the value is a string it concatenates that value into our string variable 'sentence'
        count += 1
